 ...
 */
class MarioSAM_EstimateCep_AjaxController extends Mage_Core_Controller_Front_Action 
{
    /**
    ...
    public function indexAction() 
    {
        ...
        if ($this->getRequest()->isAjax()) { //$this->getRequest()->isXmlHttpRequest()
            echo json_encode( $this->getCorreiosFrete() );
        } else {
            echo json_encode( '$this->getRequest()->isAjax(): '.$this->getRequest()->isAjax() );
        }
    }
    ...
    protected function _dadosurl( $cdServico='' ) {
        ...
        $data['nVlPeso']        = urlencode( $this->getRequest()->getParam('cepPeso', false) ) > 0 ? urlencode( $this->getRequest()->getParam('cepPeso', false) ) : 0.2;
